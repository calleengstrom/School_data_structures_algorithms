...
public:
    ...
    {
        std::string accountNo = account.getAccountNumber();
        findAndWriteToFile(accountNo);
    }

    BankAccount *findAccount(std::string accountNumber)
    {
        BankAccount *ret = nullptr;
        std::string tempRet = getAccountNo(accountNumber, account);
        *ret = tempRet;
        return ret;
    }
};
...
int main(int, char **)
{
    ...
    const int AntalAccounts = 100000;
